...
public class FbOpenGraphUrlFactory  {
	...
	public static final String OPEN_GRAPH_FEED_URL = "{{ID}}?fields=id,name,feed.limit(25).since({{since}}){from{name,picture.type(large)},id,caption,message,picture,comments.summary(true){like_count,message,id,from{name,picture.type(large)}},likes.limit(1).summary(true)}";
